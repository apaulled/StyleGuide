...
def color_outfit(closet, color):
    ...
    print(color_closet)
    outfit_result = {key: color_closet[key][randint(0, len(color_closet[key])-1)]['id'] for key in color_closet.keys()}
...
def pick_piece(drawer, colors):
    ...
    return min(drawer_diffs, key=lambda key: drawer_diffs[key][0])
